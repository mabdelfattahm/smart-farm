...
class ControllerEntity(private val record: DbRecord): Controller {
    override fun id(): ControllerId {
        return ControllerId.Default(this.record.getInt("id"))
    }

    override fun name(): ControllerName {
        return ControllerName.Default(this.record.getString("name"))
    }
    ...
    override fun status(): ControllerStatus {
        return ControllerStatus.values().first { it.ordinal == this.record.getInt("status") }
    }
}
